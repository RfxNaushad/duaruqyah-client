  ...
  const fetchSubCategories = async (catId) => {
    ...
    try {
      const response = await fetch(
        `https://dua-server-gamma.vercel.app/subcategories/${catId}`
      );
      if (!response.ok) {
  ...
  return (
    <>
      <div className="relative">
        ...
        <li
          className="flex justify-between items-center pl-4 pr-2 py-2 hover:bg-green-200 rounded-md mb-1 cursor-pointer"
...
export default function MainLeft({ categories, onSelectCategory }) {
  return (
    <div className="bg-white w-72 h-screen p-4 flex flex-col">
